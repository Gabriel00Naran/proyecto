...
import { TipoAuto } from './../../models/TipoAuto';
...
export class TipoAutoService {
   ...
   post(tipoauto: TipoAuto): Promise<any> {
      return this.http.post(this.url, JSON.stringify(tipoauto), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(tipoauto: TipoAuto): Promise<any> {
      return this.http.put(this.url, JSON.stringify(tipoauto), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
