...
import { Neumatico } from './../../models/Neumatico';
...
export class NeumaticoService {
   ...
   post(neumatico: Neumatico): Promise<any> {
      return this.http.post(this.url, JSON.stringify(neumatico), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(neumatico: Neumatico): Promise<any> {
      return this.http.put(this.url, JSON.stringify(neumatico), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
