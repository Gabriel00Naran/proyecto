...
export class UsuarioService {
   ...
   post(usuario: Usuario): Promise<any> {
      return this.http.post(this.url, JSON.stringify(usuario), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(usuario: Usuario): Promise<any> {
      return this.http.put(this.url, JSON.stringify(usuario), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
