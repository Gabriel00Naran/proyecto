...
export class EstadoService {
   ...
   post(estado: Estado): Promise<any> {
      return this.http.post(this.url, JSON.stringify(estado), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(estado: Estado): Promise<any> {
      return this.http.put(this.url, JSON.stringify(estado), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
